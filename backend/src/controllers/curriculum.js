...
exports.getCurriculums = async (req, res) => {
  try {
    ...
    let query = 'SELECT cf.*, u.name as uploader_name FROM curriculum_files cf JOIN users u ON cf.uploader_id = u.id';
    ...
    db.get(countQuery, queryParams.slice(0, -2), (err, row) => {
      if (err) {
        ...
        return res.status(500).json({ message: 'Failed to retrieve curriculum files' });
      }
  ...
  try {
    ...
    db.get(
      ...
      async (err, file) => {
        if (err) {
          console.error('Query error:', err);
          return res.status(500).json({ message: 'Failed to retrieve curriculum file' });
        }
        ...
        db.all(
          ...
          async (err, tags) => {
            if (err) {
              console.error('Get tags error:', err);
              return res.status(500).json({ message: 'Failed to retrieve file tags' });
            }
...
exports.downloadCurriculum = async (req, res) => {
  try {
    ...
    db.get('SELECT file_id FROM curriculum_files WHERE id = ?', [id], async (err, file) => {
      if (err) {
        console.error('Query error:', err);
        return res.status(500).json({ message: 'Failed to retrieve curriculum file' });
      }
      ...
      try {
        const downloadUrl = await driveService.generateDownloadUrl(file.file_id);
...
function addFileTagRelation(fileId, tagId, resolve, reject) {
  db.run('INSERT INTO file_tags (file_id, tag_id) VALUES (?, ?)', [fileId, tagId], (err) => {
    if (err) {
  ...
  db.get(
    ...
    (err, file) => {
      if (err) {
        ...
        return res.status(500).json({ message: 'Failed to retrieve uploaded file info' });
      }
    ...
    const downloadUrl = `https://drive.google.com/uc?export=download&id=google-drive-file-id-${id}`;
        ...
        db.run(
          ...
          async function(err) {
            if (err) {
              ...
              return res.status(500).json({ message: 'Failed to update curriculum file' });
            }
            
            // Handle tags if provided
            if (tags && Array.isArray(tags)) {
              // Delete existing tag associations
              db.run('DELETE FROM file_tags WHERE file_id = ?', [id], async (err) => {
                if (err) {
        ...
        db.serialize(() => {
          ...
          db.run('DELETE FROM file_tags WHERE file_id = ?', [id], (err) => {
            if (err) {
              console.error('Delete tags error:', err);
              db.run('ROLLBACK');
              return res.status(500).json({ message: 'Failed to delete curriculum file' });
            }
            
            // Delete the file entry from the database
            db.run('DELETE FROM curriculum_files WHERE id = ?', [id], async (err) => {
              if (err) {
              ...
              try {
                console.log(`Deleting file ${file.file_id} from Google Drive`);
